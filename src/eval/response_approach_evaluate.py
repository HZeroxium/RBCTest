...
def evaluate_response_property_constraint_mining(
    ...
    for api_folder in api_folders:
        if api_folder == ".DS_Store":
            continue
        approach_file = (
            f"{approach_folder}/{api_folder}/response_property_constraints.xlsx"
        )
        ground_truth_file = (
            f"{ground_truth_folder}/{api_folder}/response_property_constraints.xlsx"
        )
        ...
        ground_truth_df["concat"] = (
            ground_truth_df["attribute"]
            + ground_truth_df["description"]
            + ground_truth_df["operation"]
        )
        approach_df["concat"] = (
            approach_df["attribute"]
            + approach_df["description"]
            + approach_df["operation"]
        )
        ...
        all_fns = pd.concat(
            [
                all_fns,
                ground_truth_df[~ground_truth_df["concat"].isin(approach_df["concat"])],
            ]
        )
...
def evaluate_response_property_test_gen(
    ...
    for api_folder in api_folders:
        ...
        with open(csv_file, "a") as f:
            f.write(
                f"{api_folder},{CONSTRAINT_TYPE},{total},{total_correct},{accuracy},{len(fns)},{recall},{f1},{filter_total},{filter_correct},{filter_accuracy},{len(fns_filter)},{filter_recall},{filter_f1}\n"
            )
