...
def evaluate_request_response_constraint_mining(
    approach_folder,
    ground_truth_folder,
    api_folders,
    csv_file,
    ...
    for api_folder in api_folders:
        if api_folder == ".DS_Store":
            continue
        approach_file = (
            f"{approach_folder}/{api_folder}/request_response_constraints.xlsx"
        )
        ground_truth_file = (
            f"{ground_truth_folder}/{api_folder}/request_response_constraints.xlsx"
        )
        ...
        for row, data in approach_df.iterrows():
            ...
            attribute_inferred_from_operation = data[
                "attribute inferred from operation"
            ]

            mask = (
                (ground_truth_df["attribute"] == attribute)
                & (ground_truth_df["response resource"] == response_resource)
                & (ground_truth_df["corresponding attribute"] == parameter)
            )
            ...
            if found_mapping.empty:
                ...
                if "tp" in approach_df.columns:
                    if approach_df.at[row, "tp"] == 1:
                        approach_df.at[row, "category"] = found_mapping[
                            "category"
                        ].values[0]
                        if (
                            found_mapping["category"].values[0]
                            not in categories_dict[api_folder]
                        ):
                            categories_dict[api_folder][
                                found_mapping["category"].values[0]
                            ] = 0
                        categories_dict[api_folder][
                            found_mapping["category"].values[0]
        ...
        tmp_ground_truth_df["concat"] = (
            ...
            + tmp_ground_truth_df["response resource"]
            + tmp_ground_truth_df["corresponding attribute"]
            + tmp_ground_truth_df["attribute inferred from operation"]
        )

        tmp_approach_df = approach_df.copy()
        tmp_approach_df["concat"] = (
            tmp_approach_df["attribute"]
            + tmp_approach_df["response resource"]
            + tmp_approach_df["corresponding attribute"]
            + tmp_approach_df["attribute inferred from operation"]
        )
        ...
        all_fns = pd.concat(
            [
                all_fns,
                tmp_ground_truth_df[
                    ~tmp_ground_truth_df["concat"].isin(tmp_approach_df["concat"])
                ],
            ]
        )
        ...
        with open(csv_file, "a") as f:
            f.write(
                f"{api_folder},{CONSTRAINT_TYPE},{len(tps)},{len(fps)},{len(fns)},{precision},{recall},{f1}\n"
            )
...
def evaluate_request_response_test_gen(
    approach_folder, api_folders, csv_file, ground_truth_folder
):
    if not os.path.exists(csv_file):
        with open(csv_file, "w") as f:
            f.write(
                "API,Constraint_Type,total,total_correct,accuracy,fn,recall,f1,filter,filter_correct,filter_accuracy,filter_fn,filter_recall,filter_f1\n"
            )
    ...
    for api_folder in api_folders:
        ...
        tp = approach_df[
            (approach_df["tp"] == 1) & (approach_df["constraint_correctness"] == "TP")
        ]
        fp = approach_df[
            (approach_df["tp"] == 0) | (approach_df["constraint_correctness"] == "FP")
        ]

        temp_tp = tp.copy()
        temp_tp["concat"] = (
            temp_tp["attribute"]
            + temp_tp["response resource"]
            + temp_tp["corresponding attribute"]
            + temp_tp["attribute inferred from operation"]
        )

        temp_gt = ground_truth_df.copy()
        temp_gt["concat"] = (
            temp_gt["attribute"]
            + temp_gt["response resource"]
            + temp_gt["corresponding attribute"]
            + temp_gt["attribute inferred from operation"]
        )
        ...
        filter_tp = verified_df[
            (verified_df["tp"] == 1) & (verified_df["constraint_correctness"] == "TP")
        ]
        filter_fp = verified_df[
            (verified_df["tp"] == 0) | (verified_df["constraint_correctness"] == "FP")
        ]

        temp_filter_tp = filter_tp.copy()
        temp_filter_tp["concat"] = (
            temp_filter_tp["attribute"]
            + temp_filter_tp["response resource"]
            + temp_filter_tp["corresponding attribute"]
            + temp_filter_tp["attribute inferred from operation"]
        )

        filter_fn = temp_gt[~temp_gt["concat"].isin(temp_filter_tp["concat"])]

        print(
            f"Total: {total}, TP: {len(tp)}, FP: {len(fp)}, FN: {len(fn)}, Filter: {filter_total}, TP: {len(filter_tp)}, FP: {len(filter_fp)}, FN: {len(filter_fn)}"
        )
        ...
        with open(csv_file, "a") as f:
            f.write(
                f"{api_folder},{CONSTRAINT_TYPE},{total},{total_correct},{accuracy},{len(fn)},{recall},{f1},{filter_total},{filter_correct},{filter_accuracy},{len(filter_fn)},{filter_recall},{filter_f1}\n"
            )
